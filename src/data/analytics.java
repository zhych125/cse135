...
public class analytics {
	...
	getRecordForUser() throws Exception{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String stmtString = "SELECT u.name,p.name,sum(s.price*s.quantity) FROM tempuser u, "
				+ " tempproduct p, sales s WHERE p.pid=s.pid AND u.uid=s.uid GROUP BY u.name,p.name ";
	...
	getRecordForState() throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String stmtString = "SELECT st.state,p.name,sum(s.price*s.quantity) FROM tempuserstate st, "
				+ " tempproduct p, sales s WHERE p.pid=s.pid AND st.uid=s.uid GROUP BY st.state,p.name ";
